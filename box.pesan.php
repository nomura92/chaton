...
if($_POST[tipe]=="tutup"){
	$update="update pesan set tampil=0,buka=0 where user1='$_SESSION[username]' and user2='$_POST[u]'";$ekse=$pdo->prepare($update);$ekse->execute();
	
}elseif($_POST[tipe]=="cek"){ //jika box belum ditampilkan

$query="select a.pengirim,a.penerima,b.id_pesan from isi_pesan a,pesan b where a.penerima='$_SESSION[username]' and a.penerima=b.user1  and a.dibaca=0 and b.tampil=0 and b.buka=0 group by a.penerima";$pre=$pdo->prepare($query);$pre->execute();$cek=(int) $pre->fetch(PDO::FETCH_NUM);
if($cek>0){ //
...
}elseif($_POST[tipe]=="ceek"){ //tampilkan pemberitahuan pesan
	$query="select a.pengirim from isi_pesan a,pesan b where a.penerima='$_SESSION[username]' and a.penerima=b.user1  and a.dibaca=0 and b.tampil=0 and b.buka=0 group by a.penerima";$pre=$pdo->prepare($query);$pre->execute();$cek=(int) $pre->fetch(PDO::FETCH_NUM);
	if($cek>0){
		$query="select a.pengirim from isi_pesan a,pesan b where a.penerima='$_SESSION[username]' and a.penerima=b.user1  and a.dibaca=0 and b.tampil=0 and b.buka=0 group by a.penerima";$pre=$pdo->prepare($query);$pre->execute();$r=$pre->fetch(PDO::FETCH_ASSOC);
		echo"#data-$r[pengirim]";
	}else{
		echo"0";
	}
